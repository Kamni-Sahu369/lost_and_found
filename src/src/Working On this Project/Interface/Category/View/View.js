...
//       navigate("/login", { state: { next: "/subcategory", item } });
//       return;
//     }
//     setIsModalOpen(true);
//   };
...
import { InboxOutlined } from "@ant-design/icons";
import { useLocation, useNavigate } from "react-router-dom";
import {post_claimItem} from "../../../Api/InterfaceService"
...
function View() {
  ...
  const item = location.state?.item;
console.log(item)
  // ✅ Always place hooks BEFORE any conditional return
  useEffect(() => {
    if (location.state?.openModal && item) {
      setIsModalOpen(true);
    }
  }, [location.state, item]);
  ...
  const showModal = () => {
    ...
    if (!user) {
      alert("Please login to claim this item.");
      navigate("/loginCopy", { state: { next: "/view", item } });
      return;
    }
    setIsModalOpen(true);
  };
  ...
  formData.append("location_info", values.location_info);
  formData.append("lost_item", item.id);
  // formData.append("location_info", values.location_info);
  // ✅ Append the file (if uploaded)
  if (values.receipt_bill && values.receipt_bill.length > 0) {
    formData.append("receipt_bill", values.receipt_bill[0].originFileObj);
  }

  try {
    const response = await post_claimItem(formData);
    console.log("claim response:", response);
    alert("Claim item submitted successfully");
    form.resetFields();
    setIsModalOpen(false);
  ...
  // ✅ After hooks — safe to conditionally return
  if (!item) {
    return <div style={{ padding: "2rem", textAlign: "center" }}>No item selected</div>;
  }
            ...
            >
              <Form form={form} layout="vertical" onFinish={handleFinish}>
                <Form.Item
                  label="Describe the Item"
                  name="description"
                  rules={[{ required: true, message: "Please describe the item" }]}
                >
                  <Input.TextArea rows={3} placeholder="Color, brand, any marks..." />
                </Form.Item>

                <Form.Item
                  label="Where & When did you lose/find it?"
                  name="location_info"
                  rules={[{ required: true, message: "Location and time info is required" }]}
                >
                  <Input placeholder="E.g., Near Bus Stand, 9th June, morning" />
                </Form.Item>
